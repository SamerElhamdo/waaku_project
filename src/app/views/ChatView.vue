<template>
	<div class="h-screen flex flex-col bg-[#e5ddd5] bg-[url('data:image/svg+xml,%3Csvg width=%2260%22 height=%2260%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cdefs%3E%3Cpattern id=%22a%22 patternUnits=%22userSpaceOnUse%22 width=%2260%22 height=%2260%22 patternTransform=%22scale(0.5) rotate(0)%22%3E%3Crect x=%220%22 y=%220%22 width=%2260%22 height=%2260%22 fill=%22hsla(0,0%25,100%25,1)%22/%3E%3Cpath d=%22M30 30h30v30h-30z%22 fill=%22hsla(0,0%25,98%25,1)%22/%3E%3C/pattern%3E%3C/defs%3E%3Crect width=%22100%25%22 height=%22100%25%22 fill=%22url(%23a)%22/%3E%3C/svg%3E')]">
		<!-- Header -->
		<div class="bg-[#075e54] text-white px-4 py-3 flex items-center justify-between shadow-md z-10">
			<div class="flex items-center space-x-3 flex-1 min-w-0">
				...
				<div class="flex items-center space-x-3 flex-1 min-w-0">
					...
					<div class="flex-1 min-w-0">
						<h2 class="font-semibold text-lg truncate">{{ currentSessionId || 'Chats' }}</h2>
						<p class="text-xs text-white/80">WhatsApp</p>
					</div>
				</div>
			</div>
			<div class="flex items-center space-x-2">
				<button
					...
					:disabled="sessionExporting || !currentSessionId"
					class="p-2 hover:bg-[#0a6b5f] rounded-lg transition-colors disabled:opacity-60"
					title="Export session as JSON"
				>
					<svg
						v-if="!sessionExporting"
						class="w-6 h-6"
						fill="none"
						stroke="currentColor"
						viewBox="0 0 24 24"
		...
		<div v-if="showSessionSwitcher" class="absolute top-16 right-4 bg-white rounded-lg shadow-2xl z-50 min-w-[250px] max-h-[400px] overflow-y-auto">
			<div class="p-2">
				<div class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase">Switch Session</div>
				<div v-for="session in availableSessions" :key="session.id" 
					...
					<div class="flex items-center space-x-3">
						...
						<div class="flex-1 min-w-0">
							...
							<div class="text-xs opacity-70">{{ session.ready ? 'Ready' : 'Connecting...' }}</div>
						</div>
...
async function buildSessionExportPayload(sessionId) {
	const [sessionSnapshot, chats] = await Promise.all([
		api.exportSession(sessionId, true),
	...
	for (const chat of chats) {
		try {
			messages[chat.id] = await api.getChatMessages(sessionId, chat.id, 200)
		} catch (err) {
			messages[chat.id] = { error: api.getErrorMessage ? api.getErrorMessage(err) : err.message }
		}
	}
...
async function exportSessionBundle() {
	if (!currentSessionId.value || sessionExporting.value) return

	sessionExporting.value = true
	try {
		const payload = await buildSessionExportPayload(currentSessionId.value)
		const filename = `waaku-session-${currentSessionId.value}-${new Date().toISOString().split('T')[0]}.json`
		downloadJsonFile(filename, payload)
	} catch (err) {
		const fallbackMessage = api.getErrorMessage ? api.getErrorMessage(err) : 'Failed to export session'
